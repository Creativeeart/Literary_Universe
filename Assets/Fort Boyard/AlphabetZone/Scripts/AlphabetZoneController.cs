...
public class AlphabetZoneController : MonoBehaviour
{
    ...
    void Update()
    {
        if (FortBoyardGameController.IsAlphabetZone)
        {
            ...
            if ((waitForMoneyFalling >= 3) && (runFakeSekonds == true))
            {
                ...
                StartCoroutine(FB_CamMovingController.GoToTreasureZone()); // Переход к сокровищнице
    ...
    {
        FB_CamMovingController.CameraMovingToPoint(FB_CamMovingController.pointToAlphabetZoneB);
