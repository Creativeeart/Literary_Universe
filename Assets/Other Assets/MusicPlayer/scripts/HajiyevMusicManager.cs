...
public class HajiyevMusicManager : MonoBehaviour {
	...
	public Image playImage_Item_1;
	public Sprite playSprite;
	public Sprite pauseSprite;

	public Image playBG_Item_1;
	public Sprite playSpriteBG;
	public Sprite pauseSpriteBG;

	public Image volumeImage;
	public Sprite volume_onSprite;
	public Sprite volume_offSprite;
	[Tooltip("Clips to plays")]
	...
	public void LoadMusic(int numberObject) {
		//Pause();
		SetTrack(numberObject);
		//Play();

		//if (numberObject == 0) {
		if (IsPlaying ()) {
			
				playBG_Item_1.sprite = playSpriteBG;
				playImage_Item_1.sprite = playSprite;
				Stop ();
			
		} else {
				playBG_Item_1.sprite = pauseSpriteBG;
				playImage_Item_1.sprite = pauseSprite;
				Play ();
...
//		if (!IsPlaying()) {
//			playBG.sprite = pauseSpriteBG;
//			playImage.sprite = pauseSprite;
//			Play();
//		} else {
//			playBG.sprite = playSpriteBG;
...
//			playBG.sprite = pauseSpriteBG;
	...
	public void ToggleMute() {
		if (saveVolume < 0) {
			...
			saveVolume = volume;
			SetVolume(0);
			volumeImage.sprite = volume_offSprite;
		} else {
			SendControlMessage(Message.MusicManager_UnMute);
			SetVolume(saveVolume);
			saveVolume = -1;
			volumeImage.sprite = volume_onSprite;
		}
	}
...
//        }
    ...
    // Update is called once per frame
		...
		if (CurrentTrackNumber() >= 0) {
			string timeText = SecondsToMS(TimeInSeconds());
//			string lengthText = SecondsToMS(LengthInSeconds());

			nowPlayingText.text = "" + (CurrentTrackNumber() + 1) + ".  " + NowPlaying().name;

			musicLength.value = TimeInSeconds();
			musicLength.maxValue = LengthInSeconds();
	...
	string SecondsToMS(float seconds) {
        ...
        fadeEndVolume = Mathf.Clamp(fadeEndVolume, 0, volume);//in case volume control adjusted
                                                              //during fade
        float t = (Time.unscaledTime - fadeStartTime) / fadeTime;
        if (t >= 1) {
        	fadeTime = 0;
        	if (fadeEndVolume == 0) {
        		audioSource.Stop();
